...
export type Note = { id: string; title: string; updated_at: number; external_uri: string | null };
...
const dbPromise = SQLite.openDatabaseAsync('notes.db').then(async (db) => {
  await db.execAsync(`PRAGMA foreign_keys = ON;
    CREATE TABLE IF NOT EXISTS notes (id TEXT PRIMARY KEY, title TEXT NOT NULL, updated_at INTEGER NOT NULL, external_uri TEXT);
...
export async function listNotes(): Promise<Note[]> {
  return (await dbPromise).getAllAsync<Note>('SELECT * FROM notes ORDER BY updated_at DESC');
}
...
export async function deleteNote(id: string) {
  ...
  if (note.external_uri) {
    const { file } = await readExternal(note);
    file.delete();
  }
  await (await dbPromise).runAsync('DELETE FROM notes WHERE id = ?', id);
